...
expectType<Substitute<IUser, { name: string; address: string }>>({
  id: 1,
  name: 'hello',
  address: 'there',
});
...
expectType<Substitute<IUser, { name: [string, string]; address: string }>>({
  id: 1,
  name: ['hello', 'there'],
  address: 'miles',
});
...
expectType<RecursivePartialObj<IUser>>({ name: {} });
expectType<RecursivePartialObj<User>>({ name: { first: '' } });
expectType<RecursivePartialObj<IUser>>({ name: { first: '' } });
expectType<RecursivePartialObj<User>>({ name: { first: '', last: '' } });
expectType<RecursivePartialObj<IUser>>({ name: { first: '', last: '' } });
expectType<RecursivePartialObj<User>>({
  address: {},
});
expectType<RecursivePartialObj<IUser>>({
  address: {},
});
expectType<RecursivePartialObj<User>>({
  address: { street: '' },
...
expectType<TypeEqual<SafeOmit<IUser, 'address' | 'name'>, { id: number }>>(
  true
);
expectType<TypeEqual<SafeOmit<User, 'address'>, { id: number }>>(false);
expectType<TypeEqual<SafeOmit<IUser, 'address'>, { id: number }>>(false);
...
//@ts-expect-error should raise a TypeScript error because the key is not found
expectType<TypeEqual<SafeOmit<IUser, 'not-exist'>, User>>(true);
